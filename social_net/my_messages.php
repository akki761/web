...
$grab_messages = mysqli_query($conn,"SELECT * FROM pvt_messages WHERE user_to='$user' && opened='no' && deleted='no'");
...
<?php
//Grab the messages for the logged in user
$grab_messages = mysqli_query($conn,"SELECT * FROM pvt_messages WHERE user_to='$user' && opened='yes' && deleted='no'");
...
while ($get_msg = mysqli_fetch_assoc($grab_messages)) {
      ...
      if (@$_POST['delete_' . $id . '']) {
       $delete_msg_query = mysqli_query($conn,"UPDATE pvt_messages SET deleted='yes' WHERE id='$id'");
      }
      if (@$_POST['reply_' . $id . '']) {
